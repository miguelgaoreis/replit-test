...
public class Agent extends AbstractPlayer {
    ...
    private static final String error = "Continuous_Random";
    /**
    ...
    public Types.ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
        ...
        //action = generate_error(action);
        //... and return it.
        return actions[action];
    }

    private int generate_error(int _action){
        int action = _action;
        if (error.equals("Continuous_Random")){
            if(Math.random() < 0.05){
                action = ThreadLocalRandom.current().nextInt(0, actions.length);
            }
        }
        return action;
    }
