...
public class Chromosome implements Comparable<Chromosome>{
	...
	public ArrayList<Double> calculateFitness(long time, int number){
		if(!calculated){
			...
			//play the game using the doNothing agent and naive agent
			...
			// Workaround for broken GoalConstraint
			double correction=0;
			for(TerminationData t : SharedData.gameDescription.getTerminationConditions()){
			...
			// Assume that best Agent lost the game after 100 steps and
			// calculate the constraint fitness by applying all different constraints
			...
			// Play the game using the best agent, but only if most
			// other constraints are met (prevents wasted time)
