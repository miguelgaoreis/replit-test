package tracks.levelGeneration.architect.randomOLETS;
...
public class SingleMCTSPlayer {
    ...
    public int run(ElapsedCpuTimer elapsedTimer) {
        ...
        if (randomGenerator.nextDouble() < 0.2) {
        	int tempAction = randomGenerator.nextInt(agent.NUM_ACTIONS);
        	StateObservation tempObservation = this.rootObservation.copy();
        	tempObservation.advance(agent.actions[tempAction]);
        	if (tempObservation.getGameWinner() != Types.WINNER.PLAYER_LOSES) {
        		action = tempAction;
        	}
        }
    ...
    private void mctsSearch(ElapsedCpuTimer elapsedTimer, StateObservation rootObservation) {
        ...
        long remaining = elapsedTimer.remainingTimeMillis()-4;
        int numIters = 0;
