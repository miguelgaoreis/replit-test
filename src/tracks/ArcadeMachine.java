...
//			System.out.println(" * WARNING: Time limitations based on WALL TIME on Windows * ");
	 ...
	 */
	public static double[] runOneGame(String game_file, String level_file, boolean visuals, String agentNames,
									  String actionFile, int randomSeed, int playerID, String error, String replayer) {
		VGDLFactory.GetInstance().init(); // This always first thing to do.
		VGDLRegistry.GetInstance().init();
...
//			System.out.println(" * WARNING: Time limitations based on WALL TIME on Windows * ");
     ...
     */
    public static MapStats runGames(String game_file, String[] level_files, int level_times, String agentName, String[] actionFiles) {
	VGDLFactory.GetInstance().init(); // This always first thing to do.
	VGDLRegistry.GetInstance().init();
	...
	MapStats mapStat = new MapStats(game_file, level_files[0],new ArrayList<>());
	...
	System.out.println("Results in game " + game_file + "on map "+ level_files+ ", " + vict + " , " + sc);		//+ " , " + performance.mean());
		return mapStat;
    }
	 ...
	 */
	public static AbstractPlayer createPlayer(String playerName, String actionFile, StateObservation so,
											  int randomSeed, boolean isHuman, String error, String replayer) {
		AbstractPlayer player = null;

		try {
			// create the controller.
			player = (AbstractPlayer) createController(playerName, 0, so, error, replayer);
			if (player != null)
				player.setup(actionFile, randomSeed, isHuman);
			// else System.out.println("No controller created.");
	...
	protected static Player createController(String playerName, int playerID, StateObservation so, String error, String replayer)
			throws RuntimeException {
		Player player = null;
		try {
			...
			if (so.getNoPlayers() < 2) { // single player
				...
				Class<? extends AbstractPlayer> controllerClass = Class.forName(playerName)
						.asSubclass(AbstractPlayer.class);
				Class[] gameArgClass = new Class[] { StateObservation.class, ElapsedCpuTimer.class, String.class, String.class };
				Constructor controllerArgsConstructor = controllerClass.getConstructor(gameArgClass);

				// Call the constructor with the appropriate parameters.
				Object[] constructorArgs = new Object[] { so, ect.copy(), error, replayer };
				player = (AbstractPlayer) controllerArgsConstructor.newInstance(constructorArgs);
				player.setPlayerID(playerID);
