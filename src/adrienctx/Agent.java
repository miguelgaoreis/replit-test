...
//    private boolean gameIsPuzzle;
    ...
    public Agent(StateObservation so, ElapsedCpuTimer elapsedTimer, String error, String replayer) {
        ...
        errorGenerator = new ErrorGeneration(error, so.getAvailableActions(),replayer);
...
//            else {
        if (true) {
            ...
            Types.ACTIONS action = actions[treeSearchPlayer.returnBestAction()];
            ...
            if (treeSearchPlayer.ticksSinceFrozen > 1000) {
                ...
                Types.ACTIONS action = Types.ACTIONS.fromVector(vectorNIL);
                action = errorGenerator.generate_error(action);
                return action;
            }
        }
